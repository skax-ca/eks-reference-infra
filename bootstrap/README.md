...
IaC 밖에 두면 drift 감지·변경 이력·코드로 다시 만들 수 있다는 보장이 사라진다. 아래 표가 그
대체 수단이다.
...
최대 10회 재시도한다. 이 재시도가 없으면 첫 실행이 실패하고 두 번째 실행에서야 성공하는데,
그것은 수렴이 아니라 운이다.
...
리소스를 일부러 어긋나게 만든 뒤 `verify.sh`가 `exit 1`을 내는지 본다. 이 확인 없이는
drift 감지가 있다고 말할 수 없다.
...
이 순서대로 나오면 `verify.sh`가 drift를 잡는다는 것을 확인한 것이다. 아무것도 없는 처음
상태에서 돌리면 6건 전부 `absent`로 잡고 `exit 1`이 나오는 것도 함께 확인한다.
...
⚠️ state 버킷을 관리하는 루트의 state를 그 버킷 자신에 두면 `destroy`가 자기 state를 지운다.
승격할 때는 별도 backend 또는 `prevent_destroy`를 함께 설계한다. 지금 올리지 않는 이유가
이것이다.
