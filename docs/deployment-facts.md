...
**우리 상황:**
- 현재 핀은 전부 태그 기반이다(SSOT는 각 루트의 `main.tf` — `CLAUDE.md` §3, 여기 다시 적지 않는다).
  ⚠️ 2026-08-05 D-VERSION 재매핑 전에는 `vpc-v1.2.0`·`eks-cluster-v1.0.0`이었다 —
  **같은 커밋**이므로 shallow clone 검증 결과는 그대로 유효하다.
  🔴 **2026-08-10 정정**: 이 줄이 `eks-cluster-v0.1.0`에 멈춰 있었다. `AGENTS.md`(4곳)·`README.md`도
  같이 낡아 있었다.
  🔴 **2026-08-14 재발**: 위 정정 이후에도 `eks-cluster-v0.4.0`·`workbench-v0.4.0`으로 다시
  낡았다(실제 `main.tf`는 v0.5.0·v0.6.0). **핀 표기가 여러 파일에 복제되는 한 재발한다** — 이번엔
  숫자를 갱신하는 대신 이 줄에서 구체적 버전 나열 자체를 없앤다.
