...
# 문서다. 여기서 규칙 텍스트를 다시 쓰지 않는다. 기계로 판정 가능한 3개만 검사하고 나머지("읽는
# 사람" 첫 줄, 변경 이력 금지, 표/명령, 정정 서술 금지, 문체 규칙 전부)는 문맥 판단이 필요해
...
#
#  ⛔ 문장부호는 검사하지 않는다. em-dash 금지를 기계로 걸었다가 전면 폐기했다 - vendored 파일이
#     훼손되고, 「절 제목」 참조가 깨지고, 콜론으로 바꾼 문장이 한 문서에서만 25줄 중복됐다.
#     문장부호는 사람이 판단한다. 규칙 SSOT 에도 이 항목은 없다.
#
