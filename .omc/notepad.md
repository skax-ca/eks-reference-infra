...
## Priority Context
eks-reference-infra — hub-spoke EKS GitOps 레퍼런스 배포 루트. 2026-08-25(이어서): iac-module-library Azure 추가로 모듈 소싱 재편 예정이나 semver 태그 pin은 영향 없음 확인, 대기 예상보다 길어져 hub 재배포 진행. TGW→networking→EKS→GitOps seed 완주, 10/10 Application Synced/Healthy 확인(비밀번호 교체는 곧 재철거 예정이라 의도적 스킵). 문서 공백 2건(GH App pem 발급 절차·private gitops repo를 workbench로 clone하는 절차) 발견, project memory에 기록. 현재 hub 배포됨·dev 미배포. 다음: 사용자가 철거 시점 결정 예정.
