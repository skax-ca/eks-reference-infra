...
## Priority Context
eks-reference-infra — hub-spoke EKS GitOps 레퍼런스 배포 루트. 2026-08-26: 지난 세션에서 남긴 문서 공백 2건(GitHub App 생성 절차·private GitOps repo를 workbench로 clone하는 절차)을 GitHub 공식 문서 리서치 기반으로 scripts/README.md에 정식 문서화 완료(commit d6b01c7, hub-lifecycle.md §0에서 링크). project memory open-item 해소. 현재 hub 배포됨·dev(spoke) 미배포. 다음: 사용자가 hub 철거 시점 결정 예정(미확정, 변동 없음).
