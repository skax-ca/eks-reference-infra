...
## Priority Context
eks-reference-infra — hub-spoke EKS GitOps 레퍼런스 배포 루트. 2026-08-25: hub 전체 재구축+철거 사이클 완주(TGW→net→EKS→GitOps, teardown-verify exit 0). 실제 버그 2건 수정: live/hub/networking for_each unknown ID(4f926e8), eks-platform-gitops(별도 repo) rename 후 잔존 참조(ff44ddb). TGW destroy 순서 실측 확인 후 hub-lifecycle.md 반영(b1153cd). 현재 hub·dev(spoke) 둘 다 미배포. 다음 후보: scripts/README.md 복원(08-24 부수 삭제, git 이력에 원문 있음).
