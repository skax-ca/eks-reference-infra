...
## Priority Context
eks-reference-infra — hub-spoke EKS GitOps 레퍼런스 배포 루트. 2026-08-25: hub 재구축+철거 완주(버그 2건 수정, TGW destroy 순서 확정, b1153cd). 이어서 8/24 삭제된 README 8개 중 실제 필요한 2개(bootstrap/README.md·scripts/README.md)만 모범사례 기반 재작성, 죽은 참조 4곳 정정, 재검토로 절 번호 인용 오류 등 3건 추가 수정. live/*/README.md 4개는 코드 주석이 이미 완결적이라 의도적으로 복원 안 함. 현재 hub·dev 둘 다 미배포. 다음 후보: hub 재배포(docs/hub-lifecycle.md부터).
