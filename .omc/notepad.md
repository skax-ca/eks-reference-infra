...
## Priority Context
eks-reference-infra — hub-spoke EKS GitOps 패턴 레퍼런스이자 iac-module-library 모듈 소비 배포 루트. 2026-08-25: CLAUDE.md 재정의, docs 3종(hub/spoke-lifecycle·runbooks) TGW 누락 절차·죽은 링크 정정, 이어서 module repo docs/conventions.md §5(주석 규칙: 날짜·D번호·문서절 인용 금지) 기준으로 워크플로 YAML 5·부트스트랩 스크립트 4·live/**/*.tf 6 전체 122건 정정(커밋 f03275c, 자동 검증 스크립트는 아직 없음). 다음 세션 최우선: hub 재배포(hub-lifecycle.md 4절, team 계정에 살아있는 클러스터 없음). backend는 부분설정: 버킷명·계정ID·Role ARN 전부 git에 없음.
