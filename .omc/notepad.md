...
### 2026-08-21 04:25
### 2026-08-21 (이어서 7, 세션 마무리) — tfstate 공유 반려 근거 문서화 + 문서 버그 수정

이전 항목("RAM 초대 수락 설계 재작업")의 후속, 이 세션의 마지막 작업.

사용자가 "hub-spoke tfstate를 하나로 공유하는 방식은 왜 안 썼냐"고 질문 → 토론 끝에
사용자가 "크기·보안이 더 직접적인 이유 아니냐"고 재반박 → 검증 결과 맞음(락 경합·매
plan/apply의 state 전송 비용·CI job의 자격증명 동시 보유 3가지, 마지막 게 제일 심각).
이 판단이 지금까지 어디에도 문서화 안 돼 있던 걸 확인 → `iac-module-library`
`docs/02-choose-your-path.md` "질문 D" 절에 「갈림점 여섯 개」 표의 7번째 축(state·CI
분리)과 「state를 계정 경계에서 나누는 이유」 절을 신설(commit 024939c) → 사용자가
"갈림점 여섯 개"란 절 제목이 표 7행과 안 맞는다고 지적 → "일곱 개"로 정정(commit
c3889ec). 부수로 `live/dev/eks`에 `aws_ram_resource_share_accepter`가 없어(RAM
data source 조회만 존재) removed 블록이 불필요함을 확인·보고.

**다음 세션 착수 후보**: 없음. hub·dev(spoke) 정상 가동 중, RAM 초대 수락 설계는
CI 단계로 완전히 이관 완료, 관련 설계 결정은 전부 iac-module-library docs에 반영됨.
### 2026-08-21 06:28
### 2026-08-21 (이어서 8) — spoke teardown 재검증 + 재배포, 새 네트워크 버그 미해결 이월

이전 항목("RAM 초대 수락 설계 재작업")의 후속. 사용자 요청으로 spoke(dev) teardown 절차를 처음부터 다시 실제로 수행(destroy)해 재검증하고, 이어서 재배포까지 진행했다.

**teardown 재검증(완주, 문제 없음)**: `04-spoke-lifecycle.md` 10~13절 그대로 재현 — cluster-secret 매칭 라벨 분리 제거→root-app hard refresh→Application 8개 prune 확인→addon pod 소멸/LB·PVC 없음 확인→cluster-secret 전체 삭제→`deploy-dev-eks.yml` destroy(87개)→`deploy-dev-network.yml` destroy(69개, RAM accepter가 `removed` 블록이라 destroy 대상에서 빠진 것 확인 — 이전 세션 RAM 재설계가 실제로 작동함을 재확인)→`teardown-verify.sh`(flow-log 로그 그룹 잔존 1건, 문서화된 패턴대로 수동 삭제 후 exit 0)→hub TGW route가 blackhole로 자동 전환 확인. 문서와 어긋난 점 없음.

**재배포 중 새로 발견한 버그 2건(둘 다 처음 겪는 패턴, 아직 문서 미반영)**:

1. **hub networking 재적용 시 `aws_route.vpc_to_spoke` destroy 타임아웃(1회성, 재시도로 해결)**: `for_each` 키가 attachment ID를 포함하는데 spoke 재배포로 attachment ID가 바뀌면서 옛 키 리소스 destroy가 `waiting for Route ... delete: timeout ... context deadline exceeded`(5분 타임아웃)로 실패. AWS 실물은 이미 정상(새 attachment로 active)이었음 — 순수 API 폴링 타임아웃으로 판단, `workflow_dispatch` 재실행으로 2차는 정상 완료(0 add/0 change/2 destroy). 재발 시 그냥 재시도하면 될 가능성 높음.

2. **[미해결, 다음 세션 최우선 착수] dev EKS private endpoint에 대한 hub→spoke TCP 443 연결이 세션 중간부터 완전히 끊김**: cluster-secret 재등록→root-app refresh 직후엔 8개 Application 중 6개(cluster-autoscaler·karpenter·karpenter-nodepool·keda·kyverno-custom-policies·kyverno-policies)가 정상 Synced됐다(정상 네트워크 경로 증거). 그런데 이후 `aws-lbc`·`kyverno` 두 개만 계속 `unable to verify permissions`로 실패 — ArgoCD의 automated sync는 5회 재시도 후 "will not retry" 서킷브레이커로 자동 중단됨(이것도 이번에 처음 확인한 ArgoCD 표준 동작 — 재시도 대기는 무의미, 수동 sync 트리거 필요: `kubectl patch application <name> -n argocd --type merge -p '{"operation":{"sync":{"syncStrategy":{"hook":{}}}}}'`). 수동 재트리거 후에도 실패, 최종 에러 메시지에 `dial tcp 10.51.36.235:443: i/o timeout`이 노출됨 — dev EKS API 서버(멀티 AZ 2개 ENI: 10.51.36.235·10.51.37.99) 양쪽 IP 모두 hub workbench에서 `nc -z` 10회 전부 실패로 실측 확인. **하지만 SG·프리픽스 리스트·TGW 라우트는 전부 실측으로 정상 확인됨**(`sg-0db573926b2901585`에 `443 from pl-014cf803452cd1e4a` 존재, 프리픽스 리스트 엔트리 `10.53.0.0/16` 정확, TGW route `active`) — 설정 자체엔 결함이 없는데 실제 패킷이 안 감. workbench(uniq CIDR 대역, EC2)로 테스트한 것이라 실제 ArgoCD 파드(hub EKS 노드 위, VPC CNI SNAT 여부에 따라 소스 IP가 다를 수 있음)의 경로를 완전히 대변 못 할 가능성 있음 — 미검증. 6개 성공 후 갑자기 끊긴 타이밍(05:56Z 이후)의 원인도 미상.

**다음 세션 최우선 착수**: 이 네트워크 단절 원인 규명. 후보: ①ArgoCD 파드의 실제 소스 IP가 100.64.0.0/16(dup 대역, VPC CNI 비SNAT 시)이라 프리픽스 리스트(10.53.0.0/16 uniq만 등재)에 안 걸릴 가능성 — 파드에서 직접 `nc`/네트워크 경로 실측 필요. ②EKS private endpoint ENI 자체의 일시적 상태(신설 30분 이내). ③hub 노드의 실제 아웃바운드 SG/NACL. 확인 순서: hub EKS 아무 워크로드 파드 exec해서 파드 소스 IP 확인 → 그 IP가 dup인지 uniq인지 판정 → dup이면 프리픽스 리스트에 dup 대역도 추가할지 설계 재검토(모듈 repo 상의 필요), uniq인데도 안 되면 더 낮은 레벨(ENI/NACL) 조사. dev(spoke) EKS·networking 인프라 자체는 정상 가동 중(태그 자원 확인됨) — 파괴하지 않고 이 상태로 다음 세션 시작.
### 2026-08-23 23:52
### 2026-08-24 (이어서, 08-21 세션 네트워크 조사 계속) — hub→spoke 패킷이 TGW 구간에서 소실되는 것으로 확정

이전 항목("spoke teardown 재검증+재배포, 새 네트워크 버그 미해결 이월")의 후속. 3일 뒤 재개, 조사를 더 깊이 진행해 원인 범위를 크게 좁혔다.

**재확인 시점 증상 악화**: 8개 dev Application 전부 SYNC=Unknown(이전엔 6/8 Synced였음) — hub→spoke 크로스 계정 연결이 완전히 끊긴 상태로 악화됨.

**설정 레벨은 전부 정상임을 하나씩 실측 반증**(전부 재확인 완료, 문제 없음):
- TGW route table(tgw-rtb-044dc446b89cf8ea6): 10.51.0.0/16·10.53.0.0/16 둘 다 `active`
- 양쪽 TGW attachment: `available`, 둘 다 정확히 `tgw-rtb-044dc446b89cf8ea6`에 `associated`
- dev EKS 추가 SG(`sg-0db573926b2901585`): `443 from prefix-list pl-014cf803452cd1e4a` 정상
- 프리픽스 리스트: `10.53.0.0/16`(hub uniq) 정확, hub workbench(`10.53.10.125`)·node(`10.53.36.174`) 전부 이 대역 안에 있음
- dev EKS endpoint 서브넷(subnet-0f26507ff16e83b6a) NACL: 완전 open(default), route table도 hub 프리픽스 리스트로 TGW 라우트 정상 보유(왕복 라우팅 문제 아님)
- dev EKS 클러스터 자체: dev workbench로 직접 접속 시 완전 정상(nodes Ready, kubectl 정상) — 클러스터 자체 결함 아님, hub↔dev 경로 문제로 범위 확정

**flow log 대조로 정확한 소실 지점 특정**: hub VPC flow log(`/aws/vpc/flow-log/demo-hub-an2-main`)에는 workbench(`10.53.10.125`)·node(`10.53.36.174`, ArgoCD pod SNAT된 소스)가 `10.51.36.235:443`으로 **반복적으로 ACCEPT OK**를 내며 나가는 게 계속 기록됨(즉 hub는 계속 패킷을 내보내고 있음, SG도 허용). 그런데 dev VPC flow log(`/aws/vpc/flow-log/demo-dev-an2-main`)에는 같은 시간대에 hub 대역(10.53.x.x) 관련 트래픽이 **ACCEPT도 REJECT도 단 한 건도 없음** — 패킷이 dev VPC 경계에 도달 자체를 못 하고 있다. hub→dev 응답(SYN-ACK) 트래픽도 hub flow log에 전혀 안 잡힘(일치).

**결론**: 리소스 설정(SG·NACL·라우트·attachment 상태) 전부 정상인데 실제 패킷이 **TGW 통과 구간에서 소실**되고 있다 — API 레벨 조회로는 더 이상 원인 특정이 안 되는 지점. `/dev/tcp` 시도가 즉시 RST 없이 SYN 타임아웃(~수십초)으로 끝나는 것과도 일치(조용한 드롭 패턴).

**다음 세션 최우선 착수**: AWS **VPC Reachability Analyzer**로 hub workbench ENI → dev EKS endpoint ENI 경로 분석 실행(크로스 계정 분석 설정 필요 — RAM 공유 또는 Organization 설정 확인부터). 또는 AWS Support case 개설 검토(설정상 원인 없음을 이미 증명했으므로 AWS 인프라 레벨 이슈 가능성 배제 못 함). 참고: 이번 재배포(networking 69개·eks 87개 add) 자체는 전부 성공했고, dev(spoke) 인프라는 정상 가동 중 — 파괴하지 않고 이 상태로 유지, 조사만 이어간다. 태스크 #15(7절 완료 판정)는 여전히 미완료.
### 2026-08-24 01:28
### 2026-08-24 (spoke teardown 착수) — 10절 1단계 진행 중

세션 목적 변경 결정(별도 open-item 기록됨: 이 repo가 GitOps 절차의 새 SSOT로 전환 예정, 단
iac-module-library 문서 개편 완료 후 착수 — 이번 세션엔 미착수)과 별개로, spoke(dev)
teardown을 04-spoke-lifecycle.md 절차대로 진행 중.

**완료**: 9절(deletion_protection=false)은 이미 충족 확인(추가 조치 불필요). `iac-platform-gitops`
commit 7d42ab0으로 dev cluster-secret.yaml의 매칭 라벨(environment·tier·addon-cluster-autoscaler·
addon-keda)만 제거, secret-type·server/config·vpcName·karpenterNodeRole·region은 유지(재등록
대비). hub root-app이 이 커밋을 정확히 반영(sync.revision=7d42ab0) 확인. dev Application 7개 중
6개(aws-lbc·cluster-autoscaler·karpenter·keda·kyverno·kyverno-policies)는 정상 prune 완료.
`karpenter-nodepool` 1개만 deletionTimestamp 찍힌 채 잔류 — dev 클러스터 실물(NodePool `not found`,
Karpenter 컨트롤러 Deployment 없음, NodeClaim 없음, 시스템 관리형 노드 2개만 Ready)은 이미
깨끗함을 확인, ArgoCD Application 객체의 finalizer 해제만 지연 중으로 판단(재조정 주기 대기 중).

**다음 세션(또는 이 세션 이어서) 확인할 것**: karpenter-nodepool Application이 실제로
prune됐는지 재확인 → 10-④⑤(addon 파드·workbench IaC 밖 자원 잔존 확인, 이번엔 거의 없을 것으로
예상 — Karpenter 노드 자체가 0대였음) → 10-⑥(cluster-secret.yaml 전체 삭제) → 11절(deploy-dev-eks.yml
destroy → deploy-dev-network.yml destroy) → 12절(teardown-verify.sh AWS_PROFILE=asset) → 13절
(hub TGW 라우트 자동 blackhole, 조치 불필요).
### 2026-08-24 03:33
### 2026-08-24 (이어서 2, 세션 마무리) — spoke teardown 완주 + repo 목적 변경 결정 + hub teardown(L1~L2) 리허설 완료

이전 working memory 항목("10절 1단계 진행 중")의 후속, 이 세션의 전체 마무리.

**세션 시작 시 결정된 것(project memory open-items에 별도 기록)**: 이 repo를 "EKS GitOps 패턴의
레퍼런스"로 목적 전환하고 GitOps 절차·runbook(hub/spoke lifecycle 등)의 새 SSOT가 되기로
확정했으나, iac-module-library의 진행 중인 문서 개편 결과를 먼저 보고 나서 착수하기로 함 —
이번 세션엔 착수하지 않음.

**spoke(dev) teardown 완주(10~13절)**: cluster-secret 매칭 라벨 제거(iac-platform-gitops
commit 7d42ab0) → hub Application 8/8 prune 확인 → 그중 karpenter-nodepool 1개가
EC2NodeClass의 karpenter.k8s.aws/termination finalizer에 영구 스턱되는 **새 버그** 발견
(원인: ApplicationSet이 karpenter 컨트롤러 Application을 CR Application보다 먼저 pruned해
finalizer 처리 주체가 사라짐). AWS 실물(Launch Template) 오르판 없음 확인 후 finalizer 강제
제거로 해결(project memory gotcha 기록). cluster-secret 전체 삭제(commit 139e73e) →
deploy-dev-eks.yml destroy(plan 1m+apply 12m22s) → deploy-dev-network.yml destroy(plan 50s+
apply 2m30s) → teardown-verify.sh(AWS_PROFILE=asset) flow-log 로그 그룹 1건 수동 삭제 후
exit 0 잔존물 없음.

**hub teardown(L1~L2) 리허설 완료**: 사용자가 "hub도 teardown 시작" 요청, 범위(L1~L2만,
부트스트랩 유지)와 성격(절차 검증 리허설, 끝나면 재배포)을 AskUserQuestion으로 명확히 확인 후
진행. 11절 선처리(ArgoCD 컨트롤러 scale 0 → LB Service/Ingress/PVC 확인, 원래 0개였음 →
NodePool·EC2NodeClass 삭제, 컨트롤러 살아있는 상태라 dev 때와 달리 finalizer 문제 없이 정상
삭제) → 사용자가 "addon·ArgoCD 먼저 안 지워도 되냐" 질문 → AWS 공식 EKS 삭제 가이드+Karpenter
공식 troubleshooting 문서로 실제 검증(project memory gotcha 기록, 결론: 이 repo의 기존 절차
설계가 공식 가이드와 정확히 일치, 임시방편 아님) → deploy-hub-eks.yml destroy(plan 55s+apply
12m44s) → deploy-hub-network.yml destroy(plan 46s+apply 4m47s) → teardown-verify.sh
(AWS_PROFILE=team) flow-log 로그 그룹 1건 수동 삭제 후 exit 0 잔존물 없음.

**세션 종료 시점 인프라 상태**: team 계정에 살아있는 EKS 클러스터 없음(hub도 dev도 완전
teardown). 부트스트랩(hub의 state 버킷·OIDC provider·2단 Role)은 그대로 유지 — 다음 세션은
03-hub-lifecycle.md 4절(네트워크 L1)부터 hub 재배포로 시작(project memory open-items에
재배포 순서 상세 기록: hub networking → hub eks → argocd-seed.sh 완전 재시딩 → hub addon
확인 → spoke 재배포).
