...
provider "aws" {
  ...
  # OIDC 2단 체인의 2단째. 1단(GitHub Actions OIDC → 입구 Role iamr-demo-dev-an2-gha-entry-01)은
  # configure-aws-credentials가 워크플로에서 수행하고, 입구 Role의 권한은 이 assume 하나뿐이다.
  # 2단은 입구 Role → 실행 Role(iamr-demo-dev-an2-gha-exec-01, AdministratorAccess)이다.
