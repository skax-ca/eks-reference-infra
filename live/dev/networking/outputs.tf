...
#    (네이밍이 결정적이라 성립하는 방식 — 모듈 repo 규약). 이 출력들은 그 대안이 아니라
#    "계약이 살아 있다"의 증거다.
...
output "eks_pod_subnet_ids" {
  description = "EKS Pod 전용 서브넷 ID(ENIConfig 용). dup 대역이라 온프레미스로 라우팅되지 않는다."
  value       = module.vpc.subnet_ids_by_group["pod-dup"]
}

# 운영 라우트의 앵커. 온프레미스 대역 → TGW 같은 라우트는 이 RT 에 소비자가 직접 얹는다.
# ⚠️ 지금은 TGW 가 없어 아무것도 얹지 않는다. 앵커만 먼저 노출하는 것이 의도된 순서다.
...
output "tgw_attachment_id" {
  description = <<-EOT
    ...
    (모듈 repo docs/architectures/eks-gitops-hub-spoke/choose-your-path.md 「값 발견」 절).
    이 출력은 계약이 실제로 동작함을 보이는 용도로만 남긴다.
  EOT
