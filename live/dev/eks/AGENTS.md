...
| `main.tf` | EKS 모듈 호출 (`eks-cluster-v1.0.0`). graviton 사용 (`ami_type = AL2023_ARM_64_STANDARD`) |
...
모듈 (`eks-cluster-v1.0.0`)이 이를 상위 모듈 `terraform-aws-modules/eks` (v21.24.1)에 전달한다.
...
### 외부
- `iac-module-library` eks-cluster 모듈 (`eks-cluster-v1.0.0`)
