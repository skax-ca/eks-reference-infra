# live/hub/eks: EKS 배포 루트 (허브). networking과 독립 state(hub/eks.tfstate)다.
#
...
locals {
  # 모듈이 "eks-<workload>-<env>-<region>-<purpose>-<serial>"로 조합한다 → eks-demo-hub-an2-main-01.
  ...
  # spoke(dev)의 cross-account-trust-role ARN. spoke는 별도 계정·별도 state라 module 출력을
  # 참조할 수 없어 결정적으로 조합한다. 이름 형태는 cross-account-trust-role 모듈의
  # naming+purpose="argocd-hub" 조합과 같아야 한다(iamr-demo-dev-an2-argocd-hub).
  # ⚠️ 배포 순서는 허브가 먼저다. spoke의 trust policy는 Principal 대상이 실제로 존재해야
  #    AWS가 정책 생성을 허용하고(없으면 "Invalid principal in policy"), 이 값을 소비하는 아래
  #    argocd_hub_pod_identity는 permission policy(sts:AssumeRole on resource)라 대상 존재를
  #    검증하지 않는다.
...
module "eks" {
  ...
  # 크로스 계정 확장: 허브 ArgoCD → 스포크 EKS. 허브의 argocd_hub_pod_identity Role이 spoke의
  # cross-account-trust-role(위 local)을 assume한다. 배포 순서가 허브 먼저인 이유는 위 local 참조.
  # ⚠️ argocd_namespace는 기본값 "argocd"를 그대로 쓴다. scripts/argocd-seed.sh의 ARGOCD_NAMESPACE
  #    기본값과 일치해야 한다.
