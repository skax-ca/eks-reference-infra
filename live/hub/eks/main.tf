# live/hub/eks — EKS 배포 루트 (허브, networking 과 **독립 state**: hub/eks.tfstate)
#
...
#
# 독립 배포의 두 축:
#   ① state 분리 — backend.tf 의 키가 hub/eks.tfstate. 이 루트는 networking state 를 **읽지 않는다**.
...
locals {
  ...
  #    합성식은 live/dev/eks 에서 실계정 대조로 확인된 것과 동일한 공식이다(같은 계정·같은
  #    partition, env 토큰만 다르다) — 이 루트 자체의 실계정 대조는 첫 apply 후에 한다:
  #      arn:aws:eks:ap-northeast-2:<account>:cluster/eks-demo-hub-an2-main-01
...
module "eks" {
  source = "git::https://github.com/skax-ca/iac-module-library.git//modules/eks-cluster?ref=eks-cluster-v0.8.0&depth=1"
  ...
  cluster_addons = {
    ...
    #    workload-class=system taint를 넘지 못해 전 컴포넌트가 DEGRADED로 멈춘 것을 실측했다
    #    (`InsufficientNumberOfReplicas`, 0/2 노드 스케줄 가능 — untolerated taint).
    "cert-manager" = {
