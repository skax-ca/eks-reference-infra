...
#
# ⚠️ **왜 live/hub/networking에서 분리했나.** TGW가 hub networking과 같은
...
# destroy한 뒤 처음부터 재배포할 때 재현됨 — AWS provider에 복수형 TGW data source가
# 없고 단수형은 매칭 0건에서 하드 에러라 순수 코드 우회가 불가능함을 확인했다). TGW를 먼저
# 이 root로 세워두면, hub networking은 그 TGW를 "이미 존재하는 실물"로 data source
# 조회(known-at-plan)할 수 있다 — "레이어로 키운다"는 이 repo의 일반 원칙과 같은 이유다.
#
...
resource "aws_ec2_transit_gateway" "hub" {
  # description 은 AWS API 가 ASCII 만 허용한다(InvalidParameterValue 로 거부된 전례가 있다).
  ...
  # default_tags 가 안 붙는다(무태그로 확인됨) — 명시적으로 만든 라우트테이블은
  # provider 가 직접 만드는 리소스라 태그가 그대로 적용된다(모듈 repo
  # docs/conventions.md 의 태깅 강제 방식과 같은 원리). 자동 전파는 애초에 attachment 의 VPC 전
...
resource "aws_ram_resource_share" "tgw" {
  ...
  # 먼저 실행해야 켜진다(그 상태로 apply 하면 RAM AssociateResourceShare가
  # "Principal ... is not in your AWS organization"으로 거부한다). 관리 계정 권한이 없으므로
...
# CIDR 에도 적용한다 — 태그로는 계정 경계를 못 넘지만, RAM 이 공유하는
