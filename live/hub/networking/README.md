...
VPC 하나를 배포하는 허브 루트다. 모듈은 `iac-module-library` 에서 **git tag 로 소싱**한다(D20).
team 계정의 hub 전용 부트스트랩 자원과 state key 를 쓴다. `live/dev` 는 spoke 첫 인스턴스로
별도 계정에 배포되므로 hub 와 state 버킷·Role 을 공유하지 않는다.
...
| 무엇 | 어디서 | 어떻게 |
|------|--------|--------|
| state 버킷명 | CI: repo 변수 `HUB_TF_STATE_BUCKET` · 로컬: gitignore 된 `backend.hcl` | `tofu init -backend-config=...` |
| state key | 같음 (`hub/networking.tfstate`) | 동일 |
| 리전 / `use_lockfile` | 같음 | 동일 |
| **실행 Role ARN** | CI: repo 변수 `HUB_AWS_EXEC_ROLE_ARN` · 로컬: `TF_VAR_execution_role_arn` | provider `assume_role` |
| 입구 Role ARN | CI: repo 변수 `HUB_AWS_ENTRY_ROLE_ARN` | `configure-aws-credentials` (워크플로) |
...
> ⚠️ **왜 루트가 아니라 여기인가.** `tofu -chdir=live/hub/networking init -backend-config=backend.hcl`
...
tofu -chdir=live/hub/networking init -backend=false
...
tofu -chdir=live/hub/networking plan
...
| CIDR | 성격 | 배치 그룹 |
|------|------|-----------|
| `10.52.0.0/24` (primary) | uniq 소형 — 인프라 전용 | `ep-uniq` · `tgw-uniq` |
| `10.53.0.0/16` (secondary) | uniq — 라우팅 가능(온프레미스 도달 가정) | `pub` · `elb` · `vm` · `node` · `db` · `data` |
| `100.64.0.0/16` (secondary) | dup 허용 — 비라우팅(RFC 6598) | `pod-dup` |

> 세 대역 모두 **대상 계정에서 사용 중이 아님을 실측**하고 골랐다(2026-08-19, hub 신설 시점 전
> VPC CIDR 재조회 — `live/dev`가 쓰던 `10.50/24`·`10.51/16`은 재사용하지 않는다). 근거는
> `main.tf` 의 locals 주석에 있다.
